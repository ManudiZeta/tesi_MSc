...
$\\$
The first structures are in the vertex detector region ($\rho <$ 16 cm) and the two peaks located at the TOP edges, mainly around 118 cm (where the TOP begins) and 125 cm (where the TOP ends and the ECL starts), explicitly showing how cluster split-off and pre-showering events occur in the events. In fact, $\bar{n}$ candidates with $\bar{n}$ ($\bar{n}$ ancestor = 1), may be produced before the ECL (pre-showering) or within the ECL (split-off).
$\\$
