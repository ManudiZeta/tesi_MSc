...
$\\$
Phokhara is a Monte Carlo event generator that simulates $e^+e^-$ annihilation into hadrons at next-to-leading order (NLO) accuracy. This includes virtual and soft photon corrections to single-photon events, including double hard photon emission. 
$\\$
EvtGen is a Monte Carlo event generator that simulates decays of particles. It includes decay models for intermediate and final states with scalar, vector, and tensor mesons/resonances, as well as leptons, photons, and baryons. Decay amplitudes generate each branch of the full decay tree, accounting for angular and time-dependent correlations to simulate CP-violating processes.
$\\$
...
\end{figure}
The Phokhara\_evtgen\_combination generator produces both events with and without ISR emission, as explicitly illustrated by the TopoAna overview of generated events in Tab.~\ref{tab:topo_signal0}.
$\\$
A $vpho$ (virtual photon) is now introduced as a fictitious particle used by the generator Phokhara to produce ISR photons before the $e^+ e^-$ collisions and then enforce (EvtGen) the selected decay channel, approaching the following scheme:
...
\begin{table}[H]
    ...
    \label{tab:topo_signal0}
\end{table}

As a first step, the kinematic variables are discussed through the recoil system consisting of $p \pi^- (\gamma_{ISR})$. In parallel, the neutral $\bar{n}$ cluster candidate is reconstructed and its properties are compared to those from the recoil, using different kinematic variables. Through the recoil analysis, the anti-neutron can be identified as the missing particle in the final state.
$\\$
The first three contributes in Tab.~\ref{tab:topo_signal0} shows that events without ISR emission are the most frequently generated ($\sim 35\%$), followed by those with two ($\gamma^I\gamma^I$) ($\sim 23\%$) and one ($\gamma^I$) ISR photon ($\sim 19\%$). 
$\\$
...
\item $p$: $protonID > 0.9$, $dr<1$ cm and $|dz|<3$ cm,
...
\end{itemize}
This selection requires that the neutral $\bar{n}$ cluster candidates are reconstructed within the electromagnetic calorimeter (ECL), avoiding the $\bar{n}$ candidates from other detectors.
$\\$
...
\end{center}
Referring to Tab.~\ref{tab:topo_signal0}, 35291 events are generated without $\gamma$ emission ($iDcyTr \in {0}$) and 41706 with ISR emission ($iDcyTr = 1,2$; only the first two contributions are considered).
$\\$
...
Most of the ISR radiation is emitted in the backward ($\theta \sim 180^{\circ}$) and in the forward ($\theta \sim 0^{\circ}$) directions, where detection coverage is absent; the efficiency in the ISR case is lower than in the NO ISR case.
...
$\\$
These distribution can be compared with those of the $\bar{n}$ candidate, both from reconstruction and from generator level. 
$\\$
The $\bar{n}$ candidate list originates from neutral clusters detected in the ECL. The software assumes that all the shower energy is deposited within the ECL. Therefore, since the calorimeter measures the energy of the clusters, the $\bar{n}$ momentum $p_{\bar{n}}$ can be defined as follows:
\begin{center}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.49\textwidth]{nbar_MC/images/nbar_p_isr_conf.pdf}
    \includegraphics[width=0.49\textwidth]{nbar_MC/images/nbar_p_nog_conf.pdf}
    \caption{Momentum distribution of anti-neutron candidates, in both ISR (left) and NO ISR (right) cases.}
     \label{fig:nbar_mom}
\end{figure}
...
generator-level case, the distribution extends within [0, 6.5] GeV/c, with the same first structure extending beyond 1 GeV/c.
$\\$
...
It is evident that the majority of candidates do not have a neutron ancestor, lacking an $\bar{n}$ in their relative chain (has\_Ancestor=0), suggesting they are the primary $\bar{n}$ themselves. The second contribution is made of $\bar{n}$ candidates having $\bar{n}$ as mother etc. 
$\\$
...
$\\$
The first structures are in the vertex detector region ($\rho <$ 16 cm) and the two peaks located at the TOP edges, mainly around 118 cm (where the TOP begins) and 125 cm (where the TOP ends and the ECL starts), explicitly showing how cluster pre-showering events occur in the events. Furthermore, an extended structure can be observed for $125\,\text{cm} < \rho < 162\,\text{cm}$ (ECL region), confirming that cluster split-off effects occur within the ECL volume. Therefore, $\bar{n}$ candidates with $\bar{n}$ as mother particle may be produced before the ECL (pre-showering) or within the ECL (split-off).
$\\$
...
\begin{figure}[H]
    \centering
     ...
     \caption{Momentum and $\theta$ angle residuals distribution between the recoil system and the anti-neutron candidate. Blue: before the application of the Kinematic Fit. Red: after the application of the Kinematic Fit.}
     \label{fig:kin_fit}
\end{figure}

Looking at the standard deviation values reported in the plots, an improvement can be observed due to the application of the kinematic fit. 
$\\$
...
The obtained cluster variables in ISR case are shown in Fig.~\ref{fig:clus_vars_1}.
 \begin{figure}[H]
    ...
    \includegraphics[width=0.49\textwidth]{nbar_MC/images/clusterNHits_isr.pdf}
     ...
     \caption{Distribution of the reconstructed cluster variables for all candidates (blue) and for clusters associated to $\bar{n}$ by MC truth (red), for ISR events.}
...
The obtained cluster variables in NO ISR case are shown in Fig.~\ref{fig:clus_vars_2}.
 \begin{figure}[H]
    ...
    \caption{Distribution of the reconstructed cluster variables for all candidates (blue) and for clusters associated to $\bar{n}$ by MC truth (red), for NO ISR events.}
...
The channel $e^+ \ + \  e^- \rightarrow p \ + \ \pi^- \ + \ \bar{n} \ (+ \ \gamma_{ISR}) $ has been studied, with both ISR and NO ISR emission.
$\\$
$\\$
