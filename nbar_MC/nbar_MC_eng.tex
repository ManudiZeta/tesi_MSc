...
The event is selected by identifying the $p$, $\pi^-$ (and $\gamma_{ISR}$ for ISR case) candidates in the final state and measuring their recoil (the recoil system). 
Then the following selections are applied to $p$, $\pi^-$ and recoil system:
\begin{itemize}
...
The first one is a cut that selects a quite broad region of interest in the recoil mass distribution, corresponding to the interval [0,2] $GeV/c^2$ around the antineutron mass ($\sim 939,56$ $MeV/c^2$), requiring that the recoil direction lies within the ECL acceptance.
$\\$
...
$\\$
Parallel to the recoil system reconstruction, neutral ECL cluster $\bar{n}$ candidates are reconstructed. To analyze only $\bar{n}$ signals from the calorimeter, the following selection on $\bar{n}$ candidates is applied:
\begin{itemize}
...
To select the most probable $\bar{n}$ candidates, only those with $\alpha < 0.35$ ($\sim 20^\circ$) are stored. Therefore, the event is selected.
$\\$
...
$\\$
Since 20146 candidates are reconstructed for the ISR case and 25013 for the NO ISR case, an efficiency respectively of $\sim 71\%$ and $48\%$ can be observed.
...
$\\$
For example, considering the proton candidate list, both the reconstructed momentum and the Monte Carlo (generated) momentum of the proton list -which is not affected by the reconstruction process- are available. The latter will be referred to as the "generated" momentum.
$\\$
...
$\\$
In order to study the decay chain and kinship degree of the neutral cluster associated to $\bar{n}$ candidates detected in the ECL, the basf2 ancestor variables are used. These identify whether a given particle originates from the decay of another particle or a subsequent decay chain. The following ancestor variables are employed:
...
The variables varForFirstMCAncestorOfType(anti-n0, p) and hasAncestor(-2112, 1) of $\bar{n}$ cluster candidates are discussed. Given the close similarity between the results, only the ISR case is discussed. The hasAncestor distribution is shown in Fig. ~\ref{fig:hasAncestor}.
...
The figure Fig. ~\ref{fig:pRecoil_nmcP} can be updated, by including the momentum of the ancestors obtained with varForFirstMCAncestorOfType(anti-n0, p). The idea is to check whether the information from the momentum of the ancestors is consistent with that obtained from the primary $\bar{n}$. The obtained distributions are shown in Fig.~\ref{fig:momentum_ancestors}.
...
This can be summarized using the meta-variable (mcErrors), which encodes as a bit code the reason why the Monte Carlo matching fails. This variable explicitly illustrates the behaviour shown in Fig.~\ref{fig:mcErrors},
 \begin{figure}[h]
    ...
    \label{fig:mcErrors}
\end{figure}
where the first peak is on the code0, which means that "\emph{This particle and all its daughters are perfectly reconstructed}", while the second peak is on the code 512, which confirms that "\emph{There was an error in MC matching. Not a valid match. Might indicate fake/background track or cluster}", as expected.
