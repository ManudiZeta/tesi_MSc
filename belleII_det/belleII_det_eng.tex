...
\end{figure}
$\\$
It is noteworthy that the positron damping ring reduces the $e^+$ beam emittance, as positrons are obtained from a rotating tungsten target target bombarded by an electron beam. Electrons, by contrast, are injected directly from the linac since a controlled $e^-$ beam can be easily obtained with a Linear Accelerator (LINAC) \cite{kikuchi2005}.
$\\$
...
$\\$
The thallium-doped caesium iodide CsI(Tl) crystals have $X_0$ = 1.860 cm and $R_m$ = 3.53 cm. This material is one of the brightest known scintillators, providing a light yield Y of  $\sim 52000$ photons/MeV, with a photon emission spectrum peaked at $\sim 550$ nm, ideal for photodiode readout. Properties of typical scintillators used in calorimetry are shown in Fig.~\ref{fig:crys_prop}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{belleII_det/images/Properties-of-scintillators}
    \caption{Properties of scintillators used in calorimetry \cite{nedilko2009}.}
    \label{fig:crys_prop}
\end{figure}
...
$\\$
A table showing the ECL $\theta$ coverage and segmentation for each region is shown in Fig.~\ref{fig:ECL_coverage}.
...
$\\$
Understanding the physics processes enables identification of the main backgrounds. Selection criteria can then be further optimized by analyzing differences between the background and the signal. An example of topology diagram is provided in ~\ref{fig:topoproc}.
...
The TopoAna program \cite{2020topoana}, developed in C++ and ROOT, provides a convenient interface for verifying Monte Carlo topology information beyond scratch inspection.
$\\$
...
$\\$
TopoAna is an offline tool independent of basf2. It extracts human-readable topology information from raw MC truth data. Example of MC information, depicting the topology diagram in Fig.~\ref{fig:topoproc} (b), is shown in Fig.~\ref{fig:topoproc_a}.
\begin{figure}[H]
    \centering
...
The information from Fig.~\ref{fig:topoproc_a} may seem rather non‑intuitive to consult, so the TopoAna human-readable MC information (Fig.~\ref{fig:topoproc_b}) can be consulted instead.
