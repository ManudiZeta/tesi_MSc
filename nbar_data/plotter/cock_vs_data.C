...
void dataMC_sidebands()
{
    ...
    TString sid_cut = meet_cut + sidebands;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        TString sid_data = Form("siddata_%d", i);
        TString sid_mc   = Form("sidmc_%d", i);
        
        

        tree_1->Draw("nbar_" + var[i] + ">>" + hname_data + lim[i], sig_cut, "goff");
        tree_2->Draw("nbar_" + var[i] + ">>" + hname_mc   + lim[i], sig_cut, "goff");
        
        tree_1->Draw("nbar_" + var[i] + ">>" + sid_data + lim[i],sid_cut,"goff");
        tree_2->Draw("nbar_" + var[i] + ">>" + sid_mc + lim[i],sid_cut,"goff");

        TH1D *d1 = (TH1D*)gDirectory->Get(hname_data);
        TH1D *c1 = (TH1D*)gDirectory->Get(hname_mc);
        
        TH1D *d2 = (TH1D*)gDirectory->Get(sid_data);
        TH1D *c2 = (TH1D*)gDirectory->Get(sid_mc);
        ...
        // Salva
        TString title_out = "../images/dataMC_" + var[i] + "_" + caso + "_pull_subtracted.pdf";
        c->SaveAs(title_out);
