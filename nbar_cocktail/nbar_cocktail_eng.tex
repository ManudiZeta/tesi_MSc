...
\end{enumerate}
Furthermore, the following four additional cuts are applied beyond those used for the signal sample.
\begin{enumerate}
...
In TopoAna, each channel is identified by its own \texttt{iDcyTr} (index of the decay tree), which allows to identify clearly which decay channel has produced the selected candidate.
$\\$
...
\end{figure}
Following the optimization described above, the components in the recoil mass distribution can now be analyzed using the cut $\alpha < 0.10$ rad.
$\\$
...
As observed in the performance metrics study, the signal is more prominent relative to the background than in the previous case. Background components containing $\pi^0$ lie outside the signal region $[0.8,1.2]$ $GeV/c^2$ on the right side, while other sources exhibit a lower, flat distribution across the recoil mass distribution.
$\\$
The first 5 channels of $u\bar{u}$ TopoAna output with the $\alpha<$ 10 rad cut is shown in Fig.~\ref{fig:topo_nog_010_uubar}.
\begin{figure}[h]
    \centering
...
\begin{figure}[h]
    ...
    \caption{Top 5 channels from the TopoAna output for the $u\bar{u}$, $d\bar{d}$, $s\bar{s}$, and $c\bar{c}$ Monte Carlo cocktails (from top to bottom). NO ISR case. The \texttt{iDcyTr} index values may differ across samples, since each TopoAna output is produced independently from its corresponding ROOT ntuples.}
